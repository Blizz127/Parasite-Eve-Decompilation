...
**Phase 5EE — 171 matching C leaves; ERA COMPILER integrated (dual-toolchain build).**
...
so GCC-14.2 leaves stay byte-identical. **5EE:** `func_8003FFAC` is the first
integrated-exact `$at` absolute-`sw` setter. Its three-function pilot found a
separate return-delay scheduling blocker in `func_8007DEA4` / `func_80080930`,
which remain asm. The YAML-derived 5ED baseline was 33 short `sw` members; 32
remain, mostly type-gated. Prior era: 5ED sb+ret0 batch, 5EC
`func_800C7DC4` / `func_800C8F08` / `func_8003DFD0`, and 5EB return-0 twins.
~290 era-blocked functions remain (estimate). Prior: 5DC–5DJ added 53
