...
There are also **21 `ACCEPTED-RESIDUAL` leaves**. These are not counted as
...
The 21 leaves are represented by the current `PARKED-*` records in
...
| independent-load scheduling | `func_8006E454` |
...
registered matching-C leaves plus the existing assembly spans; the 21 residual
