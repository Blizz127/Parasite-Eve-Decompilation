...
| Port phase | **6E-B5 — FUNC_80036DC8 RUNG VERIFIED** | `pc_port/build/pe-native-tests` (174/174) |
...
| Strict mode | with `--disc-image`: exit 1 at `func_80073D24` (from `func_8003E680`) — callback-registration provider past the translated func_8003E974 + func_8003EAC8 + func_80036DC8 rungs; with `--bootstrap-disc`: fixture still aborts at `func_8007F72C` (from `func_800698D4`) | `--headless --strict-stubs --disc-image …` |
| Sanitizers | `-DPE_PORT_SANITIZERS=ON` (static libasan/libubsan): 174/174 tests + headless + real-disc load + RNG/LZCR oracle dumps clean | `pc_port/build-san` |
...
| Next | Phase 6E-B continued: `func_80073D24` rung (next strict frontier — callback registration, currently host-adapted but still routed through the bootstrap boundary; audit whether its contract is fully satisfied by the PE_Callback registry or needs SDK ResetCallback/StopCallback semantics); not MDEC/GPU/audio/input; do not start MDEC/SDL/audio/input | — |
