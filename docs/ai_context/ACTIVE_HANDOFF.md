...
## Phase 6E-B39 func_80051CC4 VERIFIED

Current verified handoff: B38 history is unchanged at
`35e8fd67ea5817883d8a5856eaf7f6ab3d52d32b` with parent
`98d94b50c7178af5e12ac8fdb006939f56f9f30b`. B39 translates
`func_80051CC4` and preserves its two unresolved dependencies on the
centralized boundary. Nothing has been pushed; `func_8005332C` and later
rungs have not been started.
...
| Port phase | **6E-B39 func_80051CC4 VERIFIED** (77 retail instructions plus 8 jump-table words independently transcribed, executable-cross-checked, and executed; true ABI `void(void)`; resource command-state initializer) | `build-port/pe-native-tests` (377/377; fresh ASan/UBSan 377/377) |
...
| Strict mode | with Disc 1: exit 1 at `func_8005332C` from `func_80051CC4`; with `--bootstrap-disc`: exit 1 at `func_8007F72C` from `func_800698D4`; normal and fresh sanitizer builds agree exactly | `--headless --strict-stubs --disc-image …` |
| Sanitizers | fresh `-DPE_PORT_SANITIZERS=ON`: 377/377 tests; bootstrap and real-disc load pass; strict frontiers agree | `/tmp/pe-b39-san.*` |
...
| Next frontier | `func_8005332C` from `func_80051CC4`; B39 stops before this dependency | — |

### Phase 6E-B39 func_80051CC4 verified — current findings
...
route `func_80053968`; types 16–18 route `func_80053B48`; both remain
centralized unresolved integer providers. The B29 oracle independently
...
hardware faithfully (reads 0x800A8038); the C port `func_80052C6C_port.c`
currently uses the literal 0x800B8038 (matching the ctest seed, which also
uses 0x800B8038). This is a documented divergence pending the real-disc
(`--disc-image`) gate confirming which address the retail image actually
populates; the two tools legitimately assert different footprints and must not
