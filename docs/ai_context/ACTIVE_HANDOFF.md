...
| VOLUME-7-79024-PARK | 287 | `func_80079024` @ `0x69824` is a proven callable 3-word handwritten GTE helper (`ctc2 a0,$26; jr ra; nop`) with five direct callers. Ordinary C emits only the return pair; the missing `ctc2` side effect has no sanctioned intrinsic and requires forbidden inline asm/macro support. `PARKED-HANDWRITTEN-COP2`; candidate stashed, no integration/count change. |
| VOLUME-8-78FAC-PARK | 287 | `func_80078FAC` @ `0x697AC` is a proven callable 3-word handwritten GTE helper (`ctc2 a0,$27; jr ra; nop`) with a direct caller. Ordinary C emits only the return pair; same `PARKED-HANDWRITTEN-COP2` expressibility mechanism as VOLUME-7. Candidate stashed; no integration/count change. |
| VOLUME-9-78FB8-PARK/STOP | 287 | `func_80078FB8` @ `0x697B8` is a proven callable 3-word handwritten GTE helper (`ctc2 a0,$28; jr ra; nop`) with a direct caller. Ordinary C emits only the return pair; `PARKED-HANDWRITTEN-COP2`, candidate stashed. This is the third consecutive park (VOLUME-7–9), so the volume campaign stops per its mandatory hard-stop rule. Final campaign result: 9 attempts, 6 matches, 3 parks; 287 leaves. |
