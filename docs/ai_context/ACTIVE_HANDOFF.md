...
| Port phase | **6E-B8 — FUNC_80029388 RUNG VERIFIED** | `pc_port/build/pe-native-tests` (202/202) |
...
| Strict mode | with `--disc-image`: exit 1 at `func_8005BCA8` (from `func_8003E680`) — the next provider past the translated func_8003E974 + func_8003EAC8 + func_80036DC8 rungs, the real func_80073D24 slot writes, the real func_800371A4 byte store, and the real func_80029388 rung; with `--bootstrap-disc`: fixture still aborts at `func_8007F72C` (from `func_800698D4`) | `--headless --strict-stubs --disc-image …` |
| Sanitizers | `-DPE_PORT_SANITIZERS=ON` (static libasan/libubsan): 202/202 tests + headless + real-disc load + RNG/LZCR/callback oracle dumps clean | `pc_port/build-san` |
...
| Next | Phase 6E-B continued: `func_8005BCA8` rung (next strict frontier, from `func_8003E680` — classify by raw MIPS before any implementation; one-rung phase); not MDEC/GPU/audio/input; do not start MDEC/SDL/audio/input | — |
