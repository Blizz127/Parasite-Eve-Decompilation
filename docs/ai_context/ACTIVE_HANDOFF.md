...
| Port phase | **6E-B2 — FUNC_80070D6C RNG ADVANCE RUNG VERIFIED** | `pc_port/build/pe-native-tests` (151/151) |
...
| Strict mode | with `--disc-image`: exit 1 at `func_8003E974` (from `func_8003E680`) — state zeroing + func_8003EAC8 allocator-style calls, past the translated RNG; with `--bootstrap-disc`: fixture still aborts at `func_8007F72C` (from `func_800698D4`) | `--headless --strict-stubs --disc-image …` |
| Sanitizers | `-DPE_PORT_SANITIZERS=ON` (static libasan/libubsan): 151/151 tests + headless + real-disc load + oracle dump clean | `pc_port/build-san` |
...
| Next | Phase 6E-B continued: provider frontier from `func_8003E974` (zeroes 0x20-word array D_800A76F0 + $gp-relative globals, then func_8003EAC8(size,flag) registration series — allocator-style bring-up; not MDEC/GPU/audio/input); do not start MDEC/SDL/audio/input | — |
