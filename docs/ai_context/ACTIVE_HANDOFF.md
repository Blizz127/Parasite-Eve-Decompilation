...
| Port phase | **6E-B16 — FUNC_8006A9E4 RUNG VERIFIED** | `pc_port/build/pe-native-tests` (251/251) |
...
| Strict mode | with `--disc-image`: exit 1 at `func_800527C8` (from `func_8006A9E4`, invoked once inside the cycle-B poll loop) — past the fully translated func_8003E680 AND the translated func_8006A9E4 streaming-load rung (real func_80074F44 ClearImage, four real read/poll cycles, real copies and lookups); with `--bootstrap-disc`: fixture still aborts at `func_8007F72C` (from `func_800698D4`) by design | `--headless --strict-stubs --disc-image …` |
| Sanitizers | `-DPE_PORT_SANITIZERS=ON` (static libasan/libubsan): 251/251 tests + headless + real-disc load + strict + RNG/LZCR/callback oracle dumps clean | `pc_port/build-san` |
...
| Next | Phase 6E-B continued: `func_800527C8` rung (next strict frontier, from `func_8006A9E4` — 49-word subsystem init with 17 callees, invoked once inside the cycle-B poll; classify by raw MIPS before any implementation; one-rung phase); then `func_80087090`; not MDEC/GPU/audio/input; do not start MDEC/SDL/audio/input | — |
