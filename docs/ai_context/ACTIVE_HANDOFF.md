...
## Phase 6E-B46 func_800851A8 PREFIX VERIFIED

The accepted B45 implementation commit is
`d72774dc86e62918612698610c70ff146721f869`. B46 translates the
deterministic prefix of `func_800851A8` (SPU DMA upload, 58 retail
words / 0xE8 bytes): completion barrier, magic number check
(`*buffer + 0xB0BEB4BF == 0`), and error path (store -1 to
D_8009D24C, return -1). The success path (SPU heap allocation via
func_80085EB4, DMA transfer via func_800850F4→func_80085E54→func_8007D9F8,
data copy to D_800B2900) requires SPU DMA hardware and routes through
the centralized bootstrap boundary. The strict real-disc frontier
advances from `func_800851A8` to its hardware success-path dependencies.
...
| Port phase | **6E-B46 func_800851A8 PREFIX VERIFIED** (58 retail words / 0xE8 bytes; magic number check + error path translated; SPU DMA success path routes through bootstrap boundary) | `build-port/pe-native-tests` (434/434 normal; fresh ASan/UBSan 434/434) |
...
| Strict mode | with Disc 1: exit 1 at func_800851A8's bootstrap boundary (success path) from func_80087090; with `--bootstrap-disc`: exit 1 at `func_8007F72C` from `func_800698D4` | `--headless --strict-stubs --disc-image …` |
...
| Next frontier | func_800851A8's success-path dependencies (func_80085EB4/func_800850F4/func_80085E54 → SPU DMA hardware); also `func_80052F24` from `func_8005218C`, `func_8007F72C` from `func_800698D4` | — |
