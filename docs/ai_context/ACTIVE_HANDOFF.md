...
## Phase 6E-B40 func_8005332C VERIFIED

Current verified base is B39 at
`fb1d62c74cd6ec06162e4896cb18f1f09f75bba3` with parent
`35e8fd67ea5817883d8a5856eaf7f6ab3d52d32b`. B40 translates only
`func_8005332C`; the later `func_8005218C` boundary remains untouched. Its
required audit also corrects the already translated `func_8005DB44` leaf;
no additional function is translated. Nothing has been pushed.
...
| Port phase | **6E-B40 func_8005332C VERIFIED** (42 retail instructions independently transcribed, executable-cross-checked, and executed; true ABI `pe_addr_t(int32_t)`; resource-record lookup) | `build-port/pe-native-tests` (387/387 normal; fresh ASan/UBSan 387/387) |
...
| Strict mode | with Disc 1: exit 1 at pre-existing `func_80053968` from `func_80053D2C`, exposed by the corrected shared lookup; with `--bootstrap-disc`: exit 1 at `func_8007F72C` from `func_800698D4`; normal and fresh sanitizer builds agree exactly | `--headless --strict-stubs --disc-image …` |
| Sanitizers | fresh `-DPE_PORT_SANITIZERS=ON`: 387/387 tests; bootstrap and real-disc load pass; strict frontiers agree | `/tmp/pe-b40-san.*` |
...
| Next frontier | `func_80053968` from `func_80053D2C`; B40 does not begin it, `func_8005218C`, or any later dependency | — |

### Phase 6E-B40 func_8005332C verified — current findings
