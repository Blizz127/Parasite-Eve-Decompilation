...
worktrees during the B54K-C rung.
...
```text
func_80030894_L6_cut (first excluded retail instruction 0x80030F6C)
...
artifact-free production-reachability rung is the post-L6 sprite setup.
...
here is the native executable's bounded headless behavior and its 934-test
...
- Native `pc_port`: capture the generic event/scheduler decision that naturally
  ...
  from `func_80030894_L6_cut` into the post-L6 sprite setup.
