...
worktrees during the B54K-B2 rung.
...
```text
func_80030894_L5_cut (first excluded retail instruction 0x80030D20)
...
artifact-free production-reachability rung is the post-L5 fixed-group setup.
...
here is the native executable's bounded headless behavior and its 932-test
...
- Native `pc_port`: capture the generic event/scheduler decision that naturally
  ...
  from `func_80030894_L5_cut` into the post-L5 fixed-group setup.
