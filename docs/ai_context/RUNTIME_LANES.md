...
worktrees during the B54K-G rung.
...
```text
func_80030894_L10_cut (first excluded retail instruction 0x80031320)
...
here is the native executable's bounded headless behavior and its 942-test
...
- Native `pc_port`: capture the generic event/scheduler decision that naturally
  ...
  from `func_80030894_L10_cut` into the fixed group at `D_8009E928`.
