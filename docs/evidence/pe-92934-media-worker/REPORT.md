...
Status: **AUTHENTICATED CARVE + MATCHED PORT** (live C89C behind Stage-1b).
...
6. **Got-frame**: bump DBC, flip 146C, then Stage-1b-gated live
   `C89C` + `7C394` (immediate pad OR `TakeStreamFrameReady`, same
   gate as `924F8`).
...
| C01C     | ported                                           |
| 91B64    | ported                                           |
| C89C     | live, Stage-1b pad / StreamFrameReady gate       |
| 7C394    | ported (behind C89C cut)                         |
| CD chain | 7C2A0 / 7F72C / 7F778 / 80D5C / 81314 ported     |
| abort    | C0D8 / 7A2A4 / 80DC4 ported; 92C48 adds 870F0    |

Match confidence: **high** on carve SHA + CFG; port matches with
Stage-1b-gated live C89C (same pad / StreamFrameReady gate as 924F8).
