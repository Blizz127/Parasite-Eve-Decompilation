# func_800C9A34 — LINK_EXACT

- Span: file `0xBA234` / VRAM `0x800C9A34` / size `0x3C` (15 words).
- Split asm: `asm/disc1/BA234.s`.
- Profile: `era_o2_g0` (`-O2 -G0`).
- Source: `src/func_800C9A34.c`.
...
LD_LIBRARY_PATH=tools/mipsel-host/usr/lib/x86_64-linux-gnu \
  ...
  tools/analysis/era_leaf_match.sh src/func_800C9A34.c 0x800C9A34 0x3C -O2 -G0
python3 tools/analysis/era_link_check.py src/func_800C9A34.c 0x800C9A34 0x3C -O2 -G0
```
...
`signed char` for the `slti 0x14` test; `the `+6` halfword` takes the `+= 0x28` step. On the
arm the `+3` byte is zeroed and `a1[1]` is set to 2.

Standard halfword family (`*(unsigned short *)(a2+4) -= 8`). Carve inside the
`0xBA234` asm block: prefix `0xBA234`, `0x3C`, resume at `0xBA270`.
