...
| file off | function | words | jr/tail | callers/refs | jal | gp | indexed symbolic/temp | loop/back-edge owner | repeated constant | boundaries | screen |
|---:|---|---:|---|---:|---:|---|---|---|---|---|---|
| 0x2FF58 | `func_8003F758` | 16 | jr-ra | 1/1 | 0 | no | - | - | - | real/real | SKIP-HANDWRITTEN-COP2: `ctc2` GTE control-register writes are not sanctioned ordinary C |
...
`func_800534CC` is removed after two attempts as `PARKED-GP-ABSOLUTE-FORM`:
its natural C forms emit an absolute address, while retail uses the `$gp+0x2D8`
small-data load. It must not consume another attempt without a new gp-form
hypothesis.
...
`func_80055FE0` is removed after two attempts as
`PARKED-BIT-TEST-CANONICALIZATION`. Retail explicitly constructs a variable
bit mask, intersects it with a gp-referenced bitset word, and converts the
result with `sltu`. Era `-O2 -G8` canonicalizes both unsigned and signed C
...
## 2026-08-30 texture-window layout overlay / hard stop

`func_800762BC` at `0x66ABC` is removed from Tier 1 after two bounded
phrasings as `PARKED-TEXTURE-WINDOW-CONTROL-FLOW-LOAD-SCHEDULE-AND-COLORING`.
The four-element component-array retry recovers retail's 16-byte frame, all
four stack homes, and exact 32-word size, but cc1 chooses the opposite null
layout, schedules Y before W, and colors X/W differently. It is the third
