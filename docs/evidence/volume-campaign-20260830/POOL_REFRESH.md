...
`634dd1b` (335-leaf base). After VOLUME-181, the matching-C count is **411**.
...
The current active universe contains **1,018** unique spans. The four-way
...
VOLUME-161 exhausted class S; VOLUME-162–181 matched the first twenty
class-B rows. The remaining closure is W=0, S=0, B=101, C=39, SKIP=4: 144
rows total. The four SKIP rows are in the suppressed class, leaving 140 attemptable
...
through VOLUME-181 removed seventy Tier-2 rows by integrating `func_800C6ED8`,
...
`func_8007F788`, `func_80080AC4`, `func_80080B04`, `func_800816F4`, and
...
`func_80050CF8`, registered callback `func_800813E8`, and slot-3 forwarding
...
callback wrappers `func_8004F950/8004F978/8004FF30/8004FF58/8004FF80/8004FFA8/8004FFD0/8004FFF8/80050038/80050060/80050204`,
leaving 962 active primary rows. The
56 active spans below were added after the primary tables through campaign overlays. Thus
`962 + 56 = 1018`; four Phase-0 rows moved from Tier 2 into SKIP, so the
...
VOLUME-120 removed exact zero-status wrapper `func_80080AE4` on its first
natural phrasing. VOLUME-121 closed the bounded ten-row probe by removing
exact constant forwarding wrapper `func_8004DC84` on its first natural
...
natural phrasing. VOLUME-127 then removed exact fixed-length equality wrapper
`func_800816F4` on its first natural phrasing. VOLUME-128 then removed exact
no-argument forwarding wrapper `func_80082534` on its first natural phrasing.
VOLUME-129 then removed exact callback-registered null-callback wrapper
`func_80050088` on its first natural phrasing. VOLUME-130 then removed exact
six-site callback-registered forwarding wrapper `func_80050260` on its first
natural phrasing. VOLUME-131 then removed exact registered callback wrapper
`func_800506E8` on its first natural phrasing. A suppressed `func_` label is
not assumed to be a retail function. VOLUME-132 then removed exact offset
callback wrapper `func_80050708` on its first natural phrasing.
VOLUME-133 then removed independently hood-proven offset twin
`func_80050728` on its first natural phrasing.
VOLUME-134 then removed exact registered callback `func_80050BE8` on its
...
first natural phrasing.
VOLUME-137 then removed exact registered callback `func_800813E8` on its first
natural phrasing.
VOLUME-138 then removed exact slot-3 forwarding wrapper `func_800824F0` on its
first natural phrasing.
VOLUME-139 then removed exact slot-4 forwarding twin `func_8007DD14` on its
first natural phrasing.
VOLUME-140 then removed exact signed-byte return wrapper `func_80052534` on
its first natural phrasing.
VOLUME-141 then removed exact signed-byte return twin `func_80052558` on its
first natural phrasing.
VOLUME-142 then removed exact GP state/boolean wrapper `func_80052790` on its
first natural phrasing.
VOLUME-143 then removed exact fixed-allocation boolean wrapper `func_8004D4A0`
...
first natural phrasing.
VOLUME-146 then removed exact GP-countdown forwarding wrapper `func_8004E94C`
on its first natural phrasing.
VOLUME-147 then removed exact three-state initializer `func_80036E34` on its
first natural phrasing.
VOLUME-148 then removed independently hood-proven adjacent initializer
...
VOLUME-161 then removed the sole class-S row `func_80039970` on its first
natural phrasing. VOLUME-162–170 removed the first nine class-B rows on their
first natural phrasings: `func_80051060`, `func_80085DC4`, and seven
independently hood-proven callback-registration wrappers
...
The next shape-ranked Tier-2 run continues in class B with
`func_800504F4` (`0x40CF4`, 10 words). Function hood and all screens must still
