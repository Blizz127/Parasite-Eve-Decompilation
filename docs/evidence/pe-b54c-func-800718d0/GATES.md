...
Matching EXE SHA-1 is unchanged: this rung edits `pc_port/` only.
...
Independent oracles that take the executable all passed (49/49
plus this rung's 8/8). `b21_bzero_oracle.py` uses `DST LEN`, not
the EXE argv, and is not in that set.

asan_ubsan=572/572, zero sanitizer diagnostics (Fedora toolbox
container, PE_PORT_SANITIZERS=ON)
