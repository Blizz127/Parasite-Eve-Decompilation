# func_800CBBF0 — LINK_EXACT

- Span: file `0xBC3F0` / VRAM `0x800CBBF0` / size `0x3C` (`15` words).
- Split asm: `asm/disc1/BC3F0.s`.
- Profile: `era_o2_g0` (`-O2 -G0`).
- Source: `src/func_800CBBF0.c`.
...
LD_LIBRARY_PATH=tools/mipsel-host/usr/lib/x86_64-linux-gnu \
  ...
  tools/analysis/era_leaf_match.sh src/func_800CBBF0.c 0x800CBBF0 0x3C -O2 -G0
python3 tools/analysis/era_link_check.py src/func_800CBBF0.c 0x800CBBF0 0x3C -O2 -G0
```
...
STEP for this leaf: **`0x3C`**.

Fourth instance of the same triplet shape (`0x3C`/`0x78`/`0xA`), in a different
overlay. Contiguous carve: prefix `0xBC3F0`, `0x3C` + `0x3C` + `0x3C`, resume at
`0xBC4A4`.
