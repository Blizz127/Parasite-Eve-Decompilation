...
int func_8006FE14(int x) {
    ...
    for (i = 0; i < 0xB; i++) {
        p = D_800942E4 + i * 0xA0C;
        if (*(int *)(p + 8) == x) {
            r = func_8006FC18(i, x, 1);
            if (r != 0)
                return r;
    ...
    for (i = 0; i < 0xB; i++) {
        p = D_800942E8 + i * 0x10C;
        if (*(int *)(p + 8) == x) {
            r = func_8006FC18(i + 0xB, x, 1);
            if (r != 0)
                return r;
            if ((unsigned int)(i + 0xB) < 0x16) {
                if ((unsigned int)(i + 0xB) >= 0xB)
                    q = D_800942E8 + ((i + 0xB) - 0xB) * 0x10C;
                else
                    q = D_800942E4 + (i + 0xB) * 0xA0C;
                if (q[1] == 0x72) {
                    unsigned int k;
                    for (k = 0x6C; k < 0x73; k++)
                        D_800E0EF0[k] = 0;
