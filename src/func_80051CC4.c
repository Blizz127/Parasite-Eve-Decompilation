 ...
 * body's global stores may alias `record`). The switch sits inside that
 * loop, so cc1's LICM hoists the loop-invariant table base and emits the
 * MATERIALISED dispatch (`lui/addiu $t6,%hi/%lo($L<n>)` before the loop,
 * then `addu $v0,$v0,$t6; lw $v0,0($v0); jr $v0`); MASPSX_DISPATCH_FOLD
 * retargets those two %hi/%lo words at jtbl_800111F8 and the dead local
 * table is stripped build-side. The clear counter and scan index share one
 * variable, which is why retail keeps both in $a0.
 *
