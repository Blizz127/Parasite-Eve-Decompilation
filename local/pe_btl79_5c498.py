#!/usr/bin/env python3
"""Does 5C498 write 4D4, D278 HP, mode, or B6A80?"""
...
def find_end(blob: bytes, start: int) -> int:
    for a in range(start, min(start + 0x800, TEXT_END), 4):
...
def main() -> None:
    ...
    print("5C498 jals", [hex(x) for x in jal_sites(blob, 0x8005C498)])
    end = find_end(blob, 0x8005C498)
    print(f"end~{end:#x} words={(end-0x8005C498)//4}")
    dump_window(blob, 0x8005C498, min(80, (end - 0x8005C498) // 4 + 4))
    # scan body for 4D4 / D278 / D28C / B6A80
    print("\n=== interesting stores in 5C498 window ===")
    for a in range(0x8005C498, end, 4):
        w = load_u32(blob, a)
        o = op(w)
        if o == 3:
            print(f"  {a:08X}  {dis(w, a)}")
        if o in (0x28, 0x29, 0x2B) and rs(w) == 28:
            print(f"  {a:08X}  {dis(w, a)}")
        if o in (0x28, 0x29, 0x2B) and simm16(w) in (0x0C, 0x0E, 0x1C, 0x4D4, 0x51C, 0x508):
            print(f"  {a:08X}  {dis(w, a)}")
