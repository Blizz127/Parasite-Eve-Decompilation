...
end = find_end(data, 0x8001CAB0)
sha = hashlib.sha256(data[exe_off(0x8001CAB0):exe_off(end)]).hexdigest()
jals = [(hex(a), hex(jal_target(load_u32(data, a)))) for a in range(0x8001CAB0, end, 4) if load_u32(data, a) >> 26 == 3]
print(f"1CAB0 {0x8001CAB0:#x}..{end:#x} {(end-0x8001CAB0)//4}w sha={sha} jals={jals}")
for addr in range(0x8001CAB0, end, 4):
