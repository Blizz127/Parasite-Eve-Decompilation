...
end = find_end(data, 0x8003A6A8)
print(f"3A6A8 end {end:#x} {(end-0x8003A6A8)//4}w")
print("=== 3A6A8 first 80 words ===")
for addr in range(0x8003A6A8, min(end, 0x8003A6A8 + 80*4), 4):
    w = load_u32(data, addr)
    print(f"  {addr:08X}  {w:08X}  {dis(w, addr)}")

e188_end = find_end(data, 0x8003E188)
sha = hashlib.sha256(data[exe_off(0x8003E188):exe_off(e188_end)]).hexdigest()
jals = [(a, jal_target(load_u32(data, a))) for a in range(0x8003E188, e188_end, 4) if load_u32(data, a) >> 26 == 3]
print(f"\n=== 3E188 {0x8003E188:#x}..{e188_end:#x} {(e188_end-0x8003E188)//4}w sha={sha} jals={jals}")
print("=== 3E188 body ===")
for addr in range(0x8003E188, e188_end, 4):
    w = load_u32(data, addr)
    print(f"  {addr:08X}  {w:08X}  {dis(w, addr)}")
