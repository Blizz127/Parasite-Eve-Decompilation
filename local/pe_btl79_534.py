#!/usr/bin/env python3
"""What writes gp+0x534 and what is 67CBC / 34DE0?"""
...
def main() -> None:
    ...
    print("=== 29A38..29A70 (534 store) ===")
    dump_window(blob, 0x80029A38, 16)
    print("\n=== gp+0x534 stores ===")
    text = blob[HDR:HDR + 0x1EE000]
    for i in range(0, len(text), 4):
        w = struct.unpack_from("<I", text, i)[0]
        if op(w) in (0x28, 0x29, 0x2B) and rs(w) == 28 and imm16(w) == 0x534:
            va = TADDR + i
            print(f"  {va:08X}  {dis(w, va)}")
    print("\n=== 67CBC ===")
    print("jals", [hex(x) for x in jal_sites(blob, 0x80067CBC)])
    dump_window(blob, 0x80067CBC, 24)
    print("\n=== 34DE0 ===")
    print("jals", [hex(x) for x in jal_sites(blob, 0x80034DE0)])
    dump_window(blob, 0x80034DE0, 24)
    print("\n=== D1CE stores ===")
    for i in range(0, len(text), 4):
        w = struct.unpack_from("<I", text, i)[0]
        if op(w) in (0x28, 0x29, 0x2B) and rs(w) == 28 and imm16(w) == 0x15E:
            # D1CE = GP+0x15E? GP+0x15E = 0x8009CECE. D1CE = 0x8009D1CE = GP+0x45E
            pass
        if op(w) in (0x28,) and rs(w) == 28 and imm16(w) == 0x45E:
            va = TADDR + i
            print(f"  {va:08X}  {dis(w, va)}")
