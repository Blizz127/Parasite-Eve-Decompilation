#!/usr/bin/env bash
# Phase 5O: Disc 1 rebuild with twelve C leaves
# (8F694, 8F868, 8F880, 8FCB4, 904A0, 90A0C, 90C38, 90C4C, 90C60, 90C74, 90F54, C2B40).
#
...
# Phase 5O file-span sizes (config subsegment edges; exclusive end).
...
run "$AS" $ASFLAGS_DEFAULT -I "$ROOT/include" -o build/asm/disc1/80CAC.s.o             asm/disc1/80CAC.s
...
# --- compile C leaves ---
step "Compile C leaves (twelve C leaves total incl. func_800C2B40)"
# shellcheck disable=SC2086
...
step "Trim ELF section alignment padding (Phase 4I/5B–5O)"
...
python3 "$TRIM" build/asm/disc1/80CAC.s.o .text "$SIZE_80CAC"
...
/* Phase 5O ROM-order link script (twelve C leaves).
...
SECTIONS
{
    ...
    .main 0x80010000 : AT(0x800)
    {
        ...
        build/asm/disc1/80CAC.s.o(.text)
        ...
        build/asm/disc1/80CAC.s.o(.data)
        ...
        build/asm/disc1/80CAC.s.o(.rodata)
...
echo "Compile:  OK (twelve C leaves with Phase 4J flags)"
...
if [[ "$cmp_ec" -eq 0 ]]; then
    ...
    echo "Matching claim: YES (twelve C leaves + remaining asm)"
