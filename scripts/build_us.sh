#!/usr/bin/env bash
# Phase 5AL: Disc 1 rebuild with thirty-four C leaves
# (17E9C, 19050, 3D82C, 3DFC8, 50D18, 527C0, 5BCA8, 8CA7C, 8F694, 8F6A8, 8F868, 8F880, 8FCB4, 904A0, 904AC, 904B4, 904BC, 906B4, 90A0C, 90C38, 90C4C, 90C60, 90C74, 90F54, C2B40, C8268, C9260, C9EA0, CACD4, CD2DC, CD2E4, CD59C, CDD04, CE3AC).
#
...
run "$AS" $ASFLAGS_DEFAULT -I "$ROOT/include" -o build/asm/disc1/9858.s.o              asm/disc1/9858.s
...
python3 "$TRIM" build/asm/disc1/9858.s.o .text "$SIZE_9858"
...
/* Phase 5AL ROM-order link script (thirty-four C leaves).
...
SECTIONS
{
    ...
    .main 0x80010000 : AT(0x800)
    {
        ...
        build/asm/disc1/9858.s.o(.rodata)
...
echo
echo "=== Summary ==="
echo "Assemble: OK (26 asm units)"
echo "Compile:  OK (thirty-four C leaves with Phase 4J flags)"
...
if [[ "$cmp_ec" -eq 0 ]]; then
    ...
    echo "Matching claim: YES (thirty-four C leaves + remaining asm)"
