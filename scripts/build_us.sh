#!/usr/bin/env bash
# Phase 5P: Disc 1 rebuild with thirteen C leaves
# (8F694, 8F868, 8F880, 8FCB4, 904A0, 904AC, 90A0C, 90C38, 90C4C, 90C60, 90C74, 90F54, C2B40).
#
...
# Phase 5P file-span sizes (config subsegment edges; exclusive end).
...
run "$AS" $ASFLAGS_DEFAULT -I "$ROOT/include" -o build/asm/disc1/80CB4.s.o             asm/disc1/80CB4.s
...
# --- compile C leaves ---
step "Compile C leaves (thirteen C leaves total incl. func_800C2B40)"
# shellcheck disable=SC2086
...
step "Trim ELF section alignment padding (Phase 4I/5B–5P)"
...
python3 "$TRIM" build/asm/disc1/80CB4.s.o .text "$SIZE_80CB4"
...
/* Phase 5P ROM-order link script (thirteen C leaves).
...
SECTIONS
{
    ...
    .main 0x80010000 : AT(0x800)
    {
        ...
        build/asm/disc1/80CB4.s.o(.text)
        ...
        build/asm/disc1/80CB4.s.o(.data)
        ...
        build/asm/disc1/80CB4.s.o(.rodata)
...
echo "Compile:  OK (thirteen C leaves with Phase 4J flags)"
...
if [[ "$cmp_ec" -eq 0 ]]; then
    ...
    echo "Matching claim: YES (thirteen C leaves + remaining asm)"
