#!/usr/bin/env bash
# Phase 5J: Disc 1 rebuild with seven C leaves
# (90A0C, 90C38, 90C4C, 90C60, 90C74, 90F54, C2B40).
#
...
# Phase 5J file-span sizes (config subsegment edges; exclusive end).
# 2A0C:     0x2A0C  → 0x8120C = 0x7E800
...
# --- compile C leaves ---
step "Compile C leaves (six 90Cxx/90F54 + func_800C2B40)"
# shellcheck disable=SC2086
...
step "Trim ELF section alignment padding (Phase 4I/5B–5J)"
...
/* Phase 5J ROM-order link script (seven C leaves).
...
echo
echo "=== Summary ==="
echo "Assemble: OK (9 asm units)"
echo "Compile:  OK (seven C leaves with Phase 4J flags)"
...
if [[ "$cmp_ec" -eq 0 ]]; then
    ...
    echo "Matching claim: YES (seven C leaves + remaining asm)"
