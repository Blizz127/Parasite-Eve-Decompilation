#!/usr/bin/env bash
# Phase 5Q: Disc 1 rebuild with fourteen C leaves
# (8F694, 8F868, 8F880, 8FCB4, 904A0, 904AC, 904B4, 90A0C, 90C38, 90C4C, 90C60, 90C74, 90F54, C2B40).
#
...
# Phase 5Q file-span sizes (config subsegment edges; exclusive end).
...
run "$AS" $ASFLAGS_DEFAULT -I "$ROOT/include" -o build/asm/disc1/80CBC.s.o             asm/disc1/80CBC.s
...
# --- compile C leaves ---
step "Compile C leaves (fourteen C leaves total incl. func_800C2B40)"
# shellcheck disable=SC2086
...
step "Trim ELF section alignment padding (Phase 4I/5B–5Q)"
...
python3 "$TRIM" build/asm/disc1/80CBC.s.o .text "$SIZE_80CBC"
...
/* Phase 5Q ROM-order link script (fourteen C leaves).
...
SECTIONS
{
    ...
    .main 0x80010000 : AT(0x800)
    {
        ...
        build/asm/disc1/80CBC.s.o(.text)
        ...
        build/asm/disc1/80CBC.s.o(.data)
        ...
        build/asm/disc1/80CBC.s.o(.rodata)
...
echo
echo "=== Summary ==="
echo "Assemble: OK (13 asm units)"
echo "Compile:  OK (fourteen C leaves with Phase 4J flags)"
echo "Pad trim: OK (incl. C .text pad strip for 0x14/0x18/0xC/0x8/0x10 bodies)"
...
if [[ "$cmp_ec" -eq 0 ]]; then
    ...
    echo "Matching claim: YES (fourteen C leaves + remaining asm)"
