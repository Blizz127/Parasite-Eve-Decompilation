#!/usr/bin/env bash
# Phase 5K: Disc 1 rebuild with eight C leaves
# (8F694, 90A0C, 90C38, 90C4C, 90C60, 90C74, 90F54, C2B40).
#
...
# Phase 5K file-span sizes (config subsegment edges; exclusive end).
# 2A0C:     0x2A0C  → 0x7FE94 = 0x7D488
# C 8F694:  0x7FE94 → 0x7FEA8 = 0x14
# 7FEA8:    0x7FEA8 → 0x8120C = 0x1364
...
# --- compile C leaves ---
step "Compile C leaves (eight C leaves total incl. func_800C2B40)"
# shellcheck disable=SC2086
...
step "Trim ELF section alignment padding (Phase 4I/5B–5K)"
...
/* Phase 5K ROM-order link script (eight C leaves).
...
echo
echo "=== Summary ==="
echo "Assemble: OK (10 asm units)"
echo "Compile:  OK (eight C leaves with Phase 4J flags)"
...
if [[ "$cmp_ec" -eq 0 ]]; then
    ...
    echo "Matching claim: YES (eight C leaves + remaining asm)"
