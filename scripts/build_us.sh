#!/usr/bin/env bash
# Phase 5L: Disc 1 rebuild with nine C leaves
# (8F694, 8F868, 90A0C, 90C38, 90C4C, 90C60, 90C74, 90F54, C2B40).
#
...
# Phase 5L file-span sizes (config subsegment edges; exclusive end).
# 2A0C:     0x2A0C  → 0x7FE94 = 0x7D488
# C 8F694:  0x7FE94 → 0x7FEA8 = 0x14
# 7FEA8:    0x7FEA8 → 0x80068 = 0x1C0
# C 8F868:  0x80068 → 0x80080 = 0x18
# 80080:    0x80080 → 0x8120C = 0x118C
...
OBJECTS=(
    ...
    "build/asm/disc1/7FEA8.s.o"
    "build/src/func_8008F868.c.o"
    "build/asm/disc1/80080.s.o"
...
run "$AS" $ASFLAGS_DEFAULT -I "$ROOT/include" -o build/asm/disc1/7FEA8.s.o             asm/disc1/7FEA8.s
run "$AS" $ASFLAGS_DEFAULT -I "$ROOT/include" -o build/asm/disc1/80080.s.o             asm/disc1/80080.s
...
# --- compile C leaves ---
step "Compile C leaves (nine C leaves total incl. func_800C2B40)"
# shellcheck disable=SC2086
...
step "Trim ELF section alignment padding (Phase 4I/5B–5L)"
...
python3 "$TRIM" build/asm/disc1/7FEA8.s.o .text "$SIZE_7FEA8"
python3 "$TRIM" build/src/func_8008F868.c.o .text "$SIZE_C_8F868"
python3 "$TRIM" build/asm/disc1/80080.s.o .text "$SIZE_80080"
python3 "$TRIM" build/src/func_80090A0C.c.o .text "$SIZE_C_LEAF"
python3 "$TRIM" build/asm/disc1/81220.s.o .text "$SIZE_81220"
# Critical: each C object .text is typically 0x20 (align-16); real body is 0x14/0x18/0x10.
...
/* Phase 5L ROM-order link script (nine C leaves).
...
SECTIONS
{
    ...
    .main 0x80010000 : AT(0x800)
    {
        ...
        build/asm/disc1/7FEA8.s.o(.text)
        build/src/func_8008F868.c.o(.text)
        build/asm/disc1/80080.s.o(.text)
        ...
        build/asm/disc1/7FEA8.s.o(.data)
        build/src/func_8008F868.c.o(.data)
        build/asm/disc1/80080.s.o(.data)
        ...
        build/asm/disc1/7FEA8.s.o(.rodata)
        build/src/func_8008F868.c.o(.rodata)
        build/asm/disc1/80080.s.o(.rodata)
        ...
        build/asm/disc1/7FEA8.s.o(.bss)
        build/src/func_8008F868.c.o(.bss)
        build/asm/disc1/80080.s.o(.bss)
...
echo
echo "=== Summary ==="
echo "Assemble: OK (11 asm units)"
echo "Compile:  OK (nine C leaves with Phase 4J flags)"
echo "Pad trim: OK (incl. C .text pad strip for 0x14/0x18/0x10 bodies)"
...
if [[ "$cmp_ec" -eq 0 ]]; then
    ...
    echo "Matching claim: YES (nine C leaves + remaining asm)"
