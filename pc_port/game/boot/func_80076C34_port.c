/*
 * Phase 6E-B53D — honest prefix of the libgpu command dispatcher
 ...
 * The prefix translated here now covers the canonical non-full path through
 * the direct-issue decision and GPUSTAT bit-26 readiness poll, stopping at
 * the unresolved guest worker call at 0x80076D38 (canonical identity
 * func_80076664).  Forced enqueue states stop earlier at the callback
 * registration helper func_80073CF4 (call at 0x80076D60); a full ring stops
 * at func_80077404 (call at 0x80076C68).  No ring entry is constructed or
 * published, no worker/pump/callback runs, and no I_MASK restore at
 * 0x80076D48/0x80076E9C is reached.
 *
...
direct_issue:
     ...
     * a0 = argument (s0) and a1 = auxiliary (s2, delay slot).  No worker
     * identity is translated yet; stop at the genuine boundary instead of
     * faking issue.  The I_MASK restore at 0x80076D48 is retail's next
     * instruction after the worker and is intentionally not reached. */
    (void)Bootstrap_ReturnInt4Indirect(
...
int func_80076C34(pe_addr_t worker, pe_addr_t argument,
                  int32_t copy_bytes, uint32_t auxiliary)
{
    return func_80076C34_prefix(worker, argument, copy_bytes, auxiliary);
}
                             ...
                             uint32_t auxiliary)
{
    ...
    /* No guest argument identity exists yet: the I_MASK exchange and
     * marker/queue decisions precede every retail use of the argument, and
     * the unresolved worker boundary records argument identity 0. */
    return func_80076C34_prefix(worker, 0u, 8, auxiliary);
}
