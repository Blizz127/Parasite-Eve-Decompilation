/*
 * decomp-source: func_80054294
 * matched VMA 0x80054294  file 0x44A94  span 0xC bytes / 3 words
 * evidence: docs/evidence/func_80054294/REPORT.md
 *
 * GENERATED by tools/analysis/gen_decomp_ports.py from src/func_80054294.c — do not
 * edit by hand; the matching leaf is the authority.  The body is copied
 * verbatim; only the host adaptation (guest-RAM data symbols + loud
 * boundaries) is generated.  See docs/ai_context/PC_PORT_FROM_DECOMP.md.
 */

#include "pe_guest_decomp.h"

#define D_8009D068 PE_DECOMP_SCALAR(0x8009D068u, int)

/* ── verbatim matching leaf (src/func_80054294.c) ───────────────────────── */

/* Phase 5DC: gp-relative leaf func_80054294.
 * VRAM 0x80054294 / gp+0x2F8 -> D_8009D068. gp base 0x8009CD70.
 * -G 8 emits R_MIPS_GPREL16; _gp+abs sym resolve to retail offset. */
/* shimmed by pe_guest_decomp.h: D_8009D068 */
int func_80054294(void){ return D_8009D068; }

