...
def main():
 ...
 cases=[];seen=set()
 for n in range(4096):
  r,index=fixture(ex,n);sp=[];regs=execute(r,0x800405A4,(index,),stop_at={0x8007DD44,0x8007DD54,0x8007DD74},visited_pcs=seen,stop_pc=sp)
 ...
 out=['/* Original card status paths; TestEvent returns (B(0Bh), mode 1000h), so the',' * frontier is the first card kernel operation (target, a0). */','static const struct {unsigned stopped;uint32_t target;uint32_t argument;uint64_t hash;} DAY1_card_status_cases[]={']
 ...
 print('PASS4096 original card status cases; TestEvent modeled, first card kernel boundary')
