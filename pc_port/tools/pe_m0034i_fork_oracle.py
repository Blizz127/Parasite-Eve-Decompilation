#!/usr/bin/env python3
"""Original M0034I sender VM forks and orders readiness/exit tasks."""
...
CHUNK=0x8018EFE8
RANGES=((0x150000,0xC00),(0x154000,0x200),(0x160000,0x180),(CHUNK&0x1FFFFF,204800),(0x9CDB4,1),(0x9CDFC,8),(0x9CE08,16),(0x9D1CC,2),(0x9D1D8,4),(0x9D1FC,4),(0x9D244,8),(0x9D264,2),(0x9D28C,4),(0x9D2E8,4),(0x9D2F8,4),(0x9D308,2),(0x9DF70,12),(0xB6A80,20),(0xB0CD8,4),(0xBCF88,4),(0xA3180,12))
def digest(r):
...
def main():
 ...
 for variant in range(16):
  for cap in (0,1,8,255):
   ...
   controller=0x80150000;aya=0x80150800;task=0x80160000;atask=task+0xC0;sender=task+0x80;child=task+0x40
   ...
   r[0x160000:0x160180]=bytes(0x180)
   sw(task,BASE+0x4DEC);sh(task+8,4);sw(task+16,1);sw(task+20,112);sw(controller+0xA8,task)
   sw(child+0x24,atask);sw(atask+0x24,atask+0x40);sw(0x8009CDFC,child);sh(0x8009D308,0xFFFF);r[0x9CDB4]=0
   sw(sender,BASE+0x46D4);sh(sender+8,variant&3);sw(sender+16,1)
   sw(sender+0x24,task+0x140 if variant&4 else 0)
   if variant&4:sw(task+0x168,sender)
   hashes=[digest(r)]
   ...
   sw(0x8009D300,sender);call(0x80017018);call(0x80065400);hashes.append(digest(r))
   assert lw(sender)==BASE+0x470C
   assert lw(child)==BASE+(0x470C if variant&3 else 0x4758)
   assert lw(child+0x24)==(task if variant&3 else task+0x140 if variant&4 else 0)
   ...
   sw(0x8009D300,child);call(0x80017018);hashes.append(digest(r))
   assert lw(child)==BASE+0x4770 and lw(child+16)==1
 ...
 assert {0x8001A918,0x80017764,0x80065400,0x80012C20,0x8001AA78,0x8001C614,0x80017AE8,0x8001A680,0x80017A50,0x800131E8,0x80012700}<=seen, sorted(hex(x) for x in seen if 0x80017900<=x<0x80017B00)
 lines=['/* Original M0034I readiness producer; real room data required. */','static const uint32_t M34F_ranges[][2]={']
 lines.extend(f'{{0x{a:X}u,{n}}},' for a,n in RANGES);lines+=['};','static const struct { unsigned variant,cap; uint64_t hash[6]; } M34F_cases[]={']
 lines.extend('{%d,%d,{%s}},'%(v,c,','.join('UINT64_C(0x%016X)'%h for h in hs)) for v,c,hs in rows);lines.append('};')
 header='\n'.join(lines)+'\n';target=ROOT/'pc_port/tests/retail_m0034i_fork_cases.h'
 if '--check' in sys.argv:assert target.read_text()==header
 else:target.write_text(header)
 (ROOT/'local/live/m0034i-fork.json').write_text(json.dumps(dict(scope='full actual sender VM and subtask creation, room mesh and Aya readiness producer, controller exit and child resumption to4770; supplied actor/root-task publication and common clip header, no scene-entry or beyond4770 proof',cases=out),indent=2)+'\n')
 print(f'PASS {len(rows)} original M0034I sender/fork graphs')
