#!/usr/bin/env python3
"""Original full mode5 cleanup, shared sparse fixtures with native tests."""
...
def make(ex,phase,v):
 ...
 sw(0x9D28C,5);sw(0x141098,(0x40000000 if v&1 else 0)|(0x40 if v&2 else 0))
...
def main():
 ...
 for phase in (0,1,2,3,255):
  for v in range(9 if phase==2 else 8):
   r,s=make(ex,phase,v);start=len(patches);patches.extend((i*4,x) for i,(x,y) in enumerate(zip(words(s),base)) if x!=y)
   execute(r,0x8002F0B0,scratchpad=bytearray(0x400),instruction_budget=1000000)
   cases.append((phase,v,start,len(patches),fingerprint(r)))
 out=[]
 for name,rows in (('ranges',RANGES),('common',common),('patches',patches)):
  out.append(f'static const uint32_t M5_{name}[][2]={{');out.extend(f'{{0x{a:X}u,0x{b:X}u}},' for a,b in rows);out.append('};')
 out.append('static const struct { unsigned phase,variant,first,end; uint64_t hash; } M5_cases[]={')
 out.extend('{%d,%d,%d,%d,UINT64_C(0x%016X)},'%c for c in cases);out.append('};')
 header='\n'.join(out)+'\n';path=ROOT/'pc_port/tests/retail_mode5_cleanup_cases.h'
 if '--check' in sys.argv:assert path.read_text()==header
 else:path.write_text(header)
 print(f'PASS {len(cases)} original full mode5 cleanup cases')
