...
Completion sets bit 26 regardless of enables. Interrupt assertion is derived
only when all three bits are set. Acknowledgement writes bit 26 while carrying
the current lower 24 bits, so unrelated represented controls are preserved.
Physical DICR master-flag bit 31 is intentionally not synthesized in B53B raw
readback: the recovered dispatcher consumes only flags 24..30, and the bounded
channel-2 assertion is exposed separately. B53C must extend readback if a newly
translated path observes bit 31. No callback or arbitrary DMA-channel
emulation is included.
...
| DICR W1C/preservation | cases 32-34; enable-off flag then asserted enable path |
