#include "retail_subdiv_model_cases.h"
static uint32_t subdiv_model_random(uint32_t *seed)
{
    *seed^=*seed<<13;*seed^=*seed>>17;*seed^=*seed<<5;return *seed;
}
static void test_DAY1_subdiv_model(void)
{
    TEST("DAY1_subdiv_model");
    static const unsigned stride[]={28,36,36,48,64,72,60,76},vertex[]={4,4,12,16,40,40,36,44};
    ...
    for(unsigned k=0;k<sizeof(DAY1_subdiv_model_cases)/sizeof(DAY1_subdiv_model_cases[0]);k++) {
        ResetTestState();memset(&g_pe_gte,0,sizeof(g_pe_gte));
        uint32_t seed=DAY1_subdiv_model_cases[k].seed,mask=DAY1_subdiv_model_cases[k].mask,mode=DAY1_subdiv_model_cases[k].mode;
        for(unsigned j=0;j<7;j++)for(unsigned i=0;i<ranges[j][1];i+=4)PE_StoreU32(ranges[j][0]+i,subdiv_model_random(&seed));
        uint32_t bias=DAY1_subdiv_model_cases[k].bias,shape=DAY1_subdiv_model_cases[k].shape;
        g_pe_gte.rt[0][0]=g_pe_gte.rt[1][1]=g_pe_gte.rt[2][2]=4096;
        g_pe_gte.tr[0]=(int32_t)(subdiv_model_random(&seed)%201)-100;
        g_pe_gte.tr[1]=(int32_t)(subdiv_model_random(&seed)%201)-100;
        g_pe_gte.tr[2]=(int32_t)(subdiv_model_random(&seed)%3000)+1000;
        ...
        if(shape){g_pe_gte.tr[0]=g_pe_gte.tr[1]=0;g_pe_gte.tr[2]=(int32_t)DAY1_subdiv_model_cases[k].depth;}
        PE_StoreU32(0x80140000,0x40);PE_StoreU32(0x80140004,0x40);
        for(unsigned kind=0;kind<8;kind++) {
            unsigned count=(mask>>kind&1)?1+subdiv_model_random(&seed)%3:0;
            ...
            for(unsigned i=0;i<3;i++) {
                ...
                unsigned order=subdiv_model_random(&seed)%4;
                ...
                if(order==3)for(unsigned j=0;j<4;j++)for(unsigned n=0;n<2;n++)coords[j][n]=(int32_t)(subdiv_model_random(&seed)%65536)-32768;
                for(unsigned j=0;j<((kind==1||kind==3||kind==5||kind==7)?4u:3u);j++) {
                    ...
                    int32_t z=(int32_t)(subdiv_model_random(&seed)%4096)-1024;
        ...
        func_80197BA0(0x80149999,0x80140040);
        ...
        if(hash!=DAY1_subdiv_model_cases[k].hash)fprintf(stderr,"subdivision model %u mask%X mode%u hash%llX/%llX\n",k,mask,mode,(unsigned long long)hash,(unsigned long long)DAY1_subdiv_model_cases[k].hash);
        ASSERT(hash==DAY1_subdiv_model_cases[k].hash,"subdivision model packets/scratch/OT/source differ from original");
        ASSERT(!memcmp(state,DAY1_subdiv_model_cases[k].state,sizeof(state)),"subdivision model GTE state differs from original");
        ASSERT(!PE_Port_ShouldStop()&&!g_stub_order_count,"subdivision model used unported behavior");
    }
    PASS();
}
